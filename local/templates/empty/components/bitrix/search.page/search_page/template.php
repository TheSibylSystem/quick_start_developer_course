...
<!-- Здесь создается форма поиска. Поле ввода q используется для ввода поискового запроса, а скрытое поле how определяет порядок сортировки результатов (по дате или релевантности) -->
...
<!-- Если есть оригинальный поисковый запрос, выводится сообщение с ссылкой на результаты по этому запросу -->
...
<? if($arResult["REQUEST"]["QUERY"] === false && $arResult["REQUEST"]["TAGS"] === false): ?>
	...
	<? elseif(count($arResult["SEARCH"])>0): ?>
		...
		<? foreach($arResult["SEARCH"] as $arItem): ?>
			...
			<small>
				<?= GetMessage('CHANGE'); ?>: <?= $arItem["DATE_CHANGE"]; ?>
			</small>
